...
namespace gazebo
{
  ...
  // Load the controller
  void WheelVelocityCalculation(double y_, double x_, double yaw, double rot_, double a, double b, double* wheel_vel)
  {
    double vy = y_ * cosf(yaw) + x_ * sinf(yaw);
    double vx = x_ * cosf(yaw) - y_ * sinf(yaw);

    wheel_vel[0]=vy - vx + rot_*(a+b);
    wheel_vel[1]=vy + vx - rot_*(a+b);
    wheel_vel[2]=vy - vx - rot_*(a+b);
    wheel_vel[3]=vy + vx + rot_*(a+b);
  ...
  void GazeboRosPlanarMove::UpdateChild()
  {
    ...
    double b = 0.3;
    double wheel_vel[4];
    WheelVelocityCalculation(y_, x_, yaw, rot_, a, b, wheel_vel);

    //补全代码,计算角速度和线速度,均由x,y,z方向的分量表示
    /*用到的变量:
    wheel_vel 数组存放 为小车四个轮子的速度，分别为右前，左前，左后，和右后
    a和b是小车中心点到轮子的宽度和长度
    ...
    */
    ...
    Vty = 1.0/4 * (wheel_vel[0] + wheel_vel[1] + wheel_vel[2] + wheel_vel[3]);
    Vtx = -1.0/4 * (wheel_vel[0] - wheel_vel[1] + wheel_vel[2] - wheel_vel[3]);
    w = 1.0/4 /(a+b) * (wheel_vel[0] - wheel_vel[1] - wheel_vel[2] + wheel_vel[3]);
     

    //END
    parent_->SetLinearVel(ignition::math::Vector3d(Vtx,Vty,0));
    parent_->SetAngularVel(ignition::math::Vector3d(0, 0, w));
